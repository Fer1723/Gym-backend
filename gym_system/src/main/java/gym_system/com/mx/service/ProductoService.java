...
public class ProductoService {
	...
	@Transactional
	public void cobrarCarrito(VentaTienda ticket) {
		for(gym_system.com.mx.entity.DetalleVenta detalle : ticket.getDetalle()) {
			detalle.setVenta(ticket);
			
			Producto p = productoRepository.findById(detalle.getIdProducto())
							.orElseThrow(() -> new RuntimeException("Error: Producto no encontrado"));
			
			if(p.getStockActual() < detalle.getCantidad()) {
				throw new RuntimeException("¡Stock insuficiente! Solo quedan " + p.getStockActual() + " de " + p.getNombre());
			}
		...
		ticket.setFechaVenta(java.time.LocalDateTime.now());
		
		try {
			gym_system.com.mx.entity.TurnoCaja turnoActual = turnoCajaService.getTurnoActivoDelUsuarioLogueado();
			ticket.setTurnoCaja(turnoActual);
			ticket.setFormaPago(gym_system.com.mx.entity.FormaPago.valueOf(ticket.getMetodoPago().toUpperCase()));
		} catch (Exception e) {
			throw new RuntimeException("No se puede procesar la venta: " + e.getMessage());
		}
		
		ventaTiendaRepository.save(ticket);
	}
